...
parser.add_argument('--models', type=str, nargs='+', default='vbpr ngcf_m grcn lattice bm3 freedom', help='choose the models')
...
def main():
    ...
    for dataset in datasets:
        for model in models:
            completed = False
            if os.path.isfile(f'{logs_path}/logfile_{dataset}_{args.mm_batch_size}_{model}.log'):
                with open(f'{logs_path}/logfile_{dataset}_{args.mm_batch_size}_{model}.log', 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()
                    completed = ('Best Model params' in content) and ('queue.Full' not in content)

            if not completed:
                command_line = (f'CUBLAS_WORKSPACE_CONFIG=:4096:8 ./run_all_elliot.sh {dataset} {args.mm_batch_size} {model} > {logs_path}/logfile_{dataset}_{args.mm_batch_size}_{model}.log 2>&1')
...
"""
    ...
    if header:
        with open(scripts_path + f'/' + f'{args.datasets.replace(" ", "_")}_{args.mm_batch_size}_{args.models.replace(" ", "_")}.sh', 'w') as f:
        ...
        with open(f'run_all_{args.datasets.replace(" ", "_")}_{args.models.replace(" ", "_")}.sh', 'w') as f:
